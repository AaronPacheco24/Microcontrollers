...
void Do_Setup_Alarm_Time(void)
{
    ...
    {   
         if (check_for_button_input() == 1)
         {
             ...
             {
                Do_Beep_Good();
                if (found == Prev) Go_Prev_Alarm_Field(); //if previous go to previous field in alarm
                if (found == Next) Go_Next_Alarm_Field();// if next go next field in alarm  
                if (found == Play_Pause) Do_Save_New_Alarm_Time();// if play pause save the data tothe alarm
                if(found == Minus) Decrease_Alarm_Time();// if minus decrease the time on the alarm
                if (found == Plus) Increase_Alarm_Time();//  if plus increase the alarm time
                if (found == EQ) Exit_Setup_Alarm_Time();// if eq exit the alarm time setup
                found = 0xff;
             }
             else
             {
                 Do_Beep_Bad(); //otherwrise do beep bad
             }
        }                   
    }
}
...
void Increase_Alarm_Time()
{
            ...
            {
                case 0:
                    setup_alarm_hour++; //increment the hour
                    if (setup_alarm_hour == 24) setup_alarm_hour = 0; //if its 24 reset back to 0
                    break;

                case 1:
                    setup_alarm_minute++; //increment the minute
                    if (setup_alarm_minute == 60) setup_alarm_minute = 0; // if minute is going to hit 60 set back to 0 
                    break;

                case 2:
                    setup_alarm_second++; // if the second is going to hit 60 set back to 0
...
{       
            ...
            {
                case 0:
                    if (setup_alarm_hour == 0) setup_alarm_hour = 23; //If we decrement from 0 go back to 23
                    else --setup_alarm_hour; //otherwise decrement
                    break;

                case 1:
                    if (setup_alarm_minute == 0) setup_alarm_minute = 59; //if we decrement from 0 go back to 50 otherwise decrement normally
                    else --setup_alarm_minute;
                    break;

                case 2:
                    if (setup_alarm_second == 0) setup_alarm_second = 59;//if we decrement from 0 go back to 50 otherwise decrement normally
...
void Update_Setup_Alarm_Time_Screen(void)
{
    ...
    setup_alarm_time[0]  = (setup_alarm_hour/10)  + '0'; //Display upper digit of the hour
    setup_alarm_time[1]  = (setup_alarm_hour%10)  + '0';// DIsplay the lower digit of the hour
    setup_alarm_time[3]  = (setup_alarm_minute/10)  + '0';// display the upper digit of the minute
    setup_alarm_time[4]  = (setup_alarm_minute%10)  + '0';// display the lower digit of the minute
    setup_alarm_time[6]  = (setup_alarm_second/10)  + '0';// display the upper digit of the second
    setup_alarm_time[7]  = (setup_alarm_second%10)  + '0';// display the lower digit of the second
