...
{ 
    ...
    strcpy(txt, " ECE3301L Sp'22 S4\0");          // Text displayed 
...
void Update_Screen(void)
{
    ...
    Heater_Set_Temp_Txt[0] = heater_set_temp/100 + '0'; //divide by 100 for the 100s number 
    Heater_Set_Temp_Txt[1] = heater_set_temp/10%10 + '0'; // Divide by 10 modulo 10 for just the 10s place
    Heater_Set_Temp_Txt[2] = heater_set_temp%10 + '0'; // modulo 10 for the ones place
    ...
    DC_Txt[0] = duty_cycle / 100 + '0'; // Divide by 100 for first digit
    DC_Txt[1] = duty_cycle /10%10 + '0'; // divide by 10 modulo 10 for second digit
    DC_Txt[2] = duty_cycle %10 + '0'; // Modulo 10 for final digit
    
    Volt_Txt[0] = ((int)  volt        ) + '0'; //The interger portion returns the first digit before the decimal
    Volt_Txt[2] = ((int)(volt*10)%10 )+ '0'; // multiply by 10 modulo 10 to shift tenths place to ones place and turn to int for just the ones place
    Volt_Txt[3] = ((int)(volt*100)%10) + '0'; // multiply by 100 modulo 10 to shift hundreths place to ones place and turn to int for just the new ones place
    
    RPM_Txt[0] = rpm / 1000+'0'; // Divide by 1000 for the largest number
    RPM_Txt[1] = rpm / 100%10 +'0'; // Divide by 100 and modulo 10 for just the hundreds number
    RPM_Txt[2] = rpm/10%10 +'0'; // divide by 10 and modulo 10 for just the 10s place
    RPM_Txt[3] = rpm%10 +'0'; // Modulo 10 for the ones place
