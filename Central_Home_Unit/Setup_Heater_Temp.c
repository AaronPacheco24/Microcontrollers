...
In_Heater_Temp_Setup = 1;
    ...
    while (In_Heater_Temp_Setup == 1)
    {
         if (check_for_button_input() == 1)
        {
             ...
             {
                Do_Beep_Good();   
                if (found == Play_Pause) Do_Save_New_Heater_Temp();// if play pause save the data of the heater
                if(found == Minus) Decrease_Heater_Temp();// if minus decrease the temperature of the heater
                if (found == Plus) Increase_Heater_Temp();//  if plus increase the temperature of the heater
                if (found == EQ) Exit_Setup_Heater_Temp();// if eq exit the heater setup
